...
function push_spider(CSRF, URL) {
	...
	if (url.length == 0 | urls.length == 0) {
		alert('Neither the interface address nor the URL link content can be empty!');
		return false
	};
	$.ajaxSetup({
		data: {
...
alert ('Interface address and sitemap address contents cannot be empty！');
		return false
	};
	$.ajaxSetup({
		data: {
...
alert ('The information to be checked and the friend chain address cannot be empty！');
		return false
	};
	$.ajaxSetup({
		data: {
	...
	$.ajax({
		...
		dataType: 'json',
		success: function(ret) {
var tab = '<table class = "table"> <thead class = "thead-light"> <tr> <th scope = "col"> Friend Link</th>' +
...
function regex_api(CSRF, URL) {
	...
	if (r.length == 0 | texts.length == 0) {
        alert ('Neither the information to be extracted nor the regular expression can be empty！');
		return false
	};
	$.ajaxSetup({
		data: {
